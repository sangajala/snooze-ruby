...
class DVDDCFTable < BaseTable
  attr_accessor :saved_path

  def initialize(browser, territory=nil)
    @browser = browser
    @tableclass = 'datesTable'
    @territory = territory
    @index = 0
    @table = nil

    if(territory==nil)
      fail("The territory of the table not given either in constructor or in current method")
    end

    @browser.elements(:class,'projectionInformation').each do |table|
      i=0
      if(table.text==@territory)
        @table = @browser.tables(:class,@tableclass)[i]
        return
      end
      i=i+1
    end
  ...
  def get_table_index(territory=nil)
    if(territory==nil)
      territory = @territory
    end
  ...
  def get_table
    @table = @browser.tables(:class,@tableclass)[get_table_index]
    return
  end
  ...
  #  end
  #  #get the cell data
  #  @table.row(:index,rownumber).cell(:index,column_number-1).text
  #end

  def get_cell_text(row_number,column_number)
    @table.row(:index,row_number).cell(:index,(column_number-1)).text
  end

  def set_cell_text(row_number,column_number,text)
    @table.row(:index,row_number).cell(:index,(column_number-1)).text_field(:index,0).set text
  end
  ...
  def get_column_headers()
    ...
    i=0
    #get all column headers
    @table.row(:index,0).cells.each do |cell|
      ar_table[i] = cell.text
      i=i+1
    end
    ar_table
  end
  ...
  def get_dvd_territory_data()
    input_data = [[]]
    for i in (0..9)
      input_data[i] = Array.new
      for j in (0..12)
        input_data[i][j] = get_cell_text(i+2,j+1)
